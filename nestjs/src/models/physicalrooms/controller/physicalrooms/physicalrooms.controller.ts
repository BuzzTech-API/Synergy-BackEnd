...
export class PhysicalroomsController {
  ...
  async createPhysicalroom(
    @Body() createPhysicalroomDto: CreatePhysicalroomDto,
  ) {
    const newPhysicalroom = await this.physicalroomsService.createPhysicalroom(
  ...
  async getPhysicalroomById(@Param('id', ParseIntPipe) id: number) {
    // usa os id da rota (ParseIntPipe): garante que o id é um numero
    const physicalroom =
      await this.physicalroomsService.getPhysicalroomById(id);
    return physicalroom;
  }

  @HttpCode(200)
  @Get('/reservations/:id') // physicalrooms/reservations/{id}
  async getPhysicalroomReservationsById(@Param('id', ParseIntPipe) id: number) {
    // usa os id da rota (ParseIntPipe): garante que o id é um numero
    const physicalroom =
      await this.physicalroomsService.getPhysicalroomReservationsById(id);
    return physicalroom;
  }
  ...
  async updatePhysicalRoom(
    ...
    const physicalroom = await this.physicalroomsService.updatePhysicalRoom(
